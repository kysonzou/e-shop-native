...
func NewHTTPServer(c *conf.Server) *BusinessHTTPServer {
	// 初始化gateway
	...
	chi := chi.NewRouter()
	//chi.Use() //可以挂载各种中间件
	chi.Use(middleware.Recoverer) // 1. 终极保护，必须在最外层之一，捕获一切panic
	chi.Use(middleware.Logger)    // 2. 记录请求的完整生命周期
	chi.Use(MetricsMiddleware)
